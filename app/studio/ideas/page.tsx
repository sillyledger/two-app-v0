...
import { Search, Plus, List, LayoutGrid, MoreVertical, Trash2, Pencil, ChevronRight } from 'lucide-react'
...
export default function IdeasPage() {
  ...
  const renderSuggestionDropdown = (idea: ContentIdea, field: 'platform' | 'category', options: string[], top: number) => {
    ...
    return (
      <div onMouseDown={e => e.preventDefault()} style={{ position: 'absolute', left: 0, top, zIndex: 50, minWidth: 170, borderRadius: 10, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', padding: '4px 0', overflow: 'hidden', background: 'var(--menu-bg)', border: '1px solid var(--border-subtle)' }}>
        {suggestions.map(opt => (
          renamingSuggestion?.field === field && renamingSuggestion.value === opt ? (
            <input
              ...
              style={{ display: 'block', width: 'calc(100% - 16px)', margin: '2px 8px', fontSize: 12.5, fontFamily: FONT, background: 'var(--surface-2)', border: '1px solid var(--border-subtle)', borderRadius: 6, color: 'var(--text-1)', padding: '5px 6px' }}
            />
          ) : (
            ...
            >
              <button
                ...
                style={{ flex: 1, minWidth: 0, textAlign: 'left', fontSize: 12.5, fontFamily: FONT, color: 'var(--text-2)', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}
                className="truncate"
              >
                {opt}
              </button>
              <div style={{ display: 'flex', gap: 2, flexShrink: 0, opacity: 0.5 }}>
                <button onClick={() => startRenameSuggestion(field, opt)} style={{ width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: 'var(--text-3)', cursor: 'pointer', borderRadius: 4 }}>
                  <Pencil size={11} />
                </button>
                <button onClick={() => handleDeleteSuggestion(field, opt)} style={{ width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', color: 'var(--danger)', cursor: 'pointer', borderRadius: 4 }}>
                  <Trash2 size={11} />
                </button>
              </div>
            </div>
  ...
  const renderStatusControl = (idea: ContentIdea) => (
    <div style={{ position: 'relative' }} ref={statusMenuId === idea.id ? statusMenuRef : undefined}>
      ...
      {statusMenuId === idea.id && (
        <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', left: 0, top: 22, zIndex: 50, width: 140, borderRadius: 10, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', padding: '4px 0', overflow: 'hidden', background: 'var(--menu-bg)', border: '1px solid var(--border-subtle)' }}>
          {STATUS_ORDER.map(s => (
            <button
              key={s}
              onClick={() => handleSetStatus(idea, s)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 12px', fontSize: 12.5, fontFamily: FONT, color: 'var(--text-2)', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' }}
              onMouseEnter={e => (e.currentTarget.style.background = 'var(--hover)')}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
            >
    ...
    <div style={{ position: 'relative' }} ref={typeMenuId === idea.id ? typeMenuRef : undefined}>
      {idea.type ? (
        <button
          ...
          style={{ fontSize: 12, fontFamily: FONT, padding: 0, background: 'transparent', color: TYPE_META[idea.type]?.color ?? 'var(--text-3)', border: 'none', cursor: 'pointer' }}
        >
        ...
        <button
          onClick={e => { e.stopPropagation(); setTypeMenuId(prev => prev === idea.id ? null : idea.id) }}
          style={{ fontSize: 12, fontFamily: FONT, color: 'var(--text-3)', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}
        >
          + Type
        </button>
      )}
      {typeMenuId === idea.id && (
        <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', left: 0, top: 22, zIndex: 50, width: 120, borderRadius: 10, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', padding: '4px 0', overflow: 'hidden', background: 'var(--menu-bg)', border: '1px solid var(--border-subtle)' }}>
          {TYPE_ORDER.map(t => (
            <button
              key={t}
        ...
        <div className="max-w-[1180px] mx-auto px-10 py-10">
          <div className="flex items-center flex-wrap gap-1 mb-3 text-[12px]" style={{ color: 'var(--text-3)' }}>
            ...
            <span className="flex items-center gap-1">
              ...
              <span className="font-semibold" style={{ color: 'var(--text-1)' }}>Ideas</span>
            </span>
          </div>
          ...
          <div className="flex items-center justify-between mb-6 gap-4">
            <div className="relative flex-1" style={{ maxWidth: 420 }}>
              ...
              <input
                ...
                style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border-subtle)', color: 'var(--text-1)', fontFamily: FONT }}
              />
            </div>
            <div className="flex items-center gap-2.5" style={{ flexShrink: 0 }}>
              <div style={{ display: 'flex', border: '1px solid var(--border-subtle)', borderRadius: 8, overflow: 'hidden' }}>
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 11px', fontSize: 12.5, fontFamily: FONT, border: 'none', borderRight: '1px solid var(--border-subtle)', cursor: 'pointer', background: view === 'list' ? 'var(--surface-2)' : 'transparent', color: view === 'list' ? 'var(--text-1)' : 'var(--text-3)' }}
                >
                  <List size={14} /> List
                </button>
                <button
                  onClick={() => setView('board')}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 11px', fontSize: 12.5, fontFamily: FONT, border: 'none', cursor: 'pointer', background: view === 'board' ? 'var(--surface-2)' : 'transparent', color: view === 'board' ? 'var(--text-1)' : 'var(--text-3)' }}
                >
              ...
              <button
                ...
                style={{ backgroundColor: 'var(--primary-bg)', color: 'var(--primary-text)', fontFamily: FONT }}
              >
          ...
          <p className="text-sm mb-6" style={{ color: 'var(--text-3)', fontFamily: FONT }}>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 gap-2">
              <p className="text-sm" style={{ color: 'var(--text-3)' }}>{trimmedQuery ? 'No ideas match your search' : 'No ideas yet'}</p>
              <p className="text-xs" style={{ color: 'var(--text-3)' }}>{trimmedQuery ? 'Try a different search term' : 'Click New idea to get started'}</p>
            </div>
          ) : view === 'list' ? (
            <div className="flex flex-col">
              {filteredIdeas.map(idea => (
                <div
                  key={idea.id}
                  className="group"
                  style={{ display: 'grid', gridTemplateColumns: GRID_COLS, alignItems: 'center', gap: 12, borderBottom: '1px solid var(--border-subtle)', padding: '14px 8px', borderRadius: 8 }}
                ...
                >
                  {editingField?.id === idea.id && editingField.field === 'title' ? (
                    <input
                      ...
                      style={{ minWidth: 0, fontSize: 14, fontWeight: 500, lineHeight: 1.4, fontFamily: FONT, background: 'var(--surface-2)', border: '1px solid var(--border-subtle)', borderRadius: 6, color: 'var(--text-1)', padding: '2px 6px' }}
                    />
                  ) : (
                    <div
                      className="truncate"
                      onClick={() => startEdit(idea, 'title')}
                      style={{ minWidth: 0, fontSize: 14, fontWeight: 500, lineHeight: 1.4, fontFamily: FONT, color: 'var(--text-1)', cursor: 'text' }}
                    >
                      {idea.title}
                    </div>
                  )}
                  ...
                  {editingField?.id === idea.id && editingField.field === 'platform' ? (
                    <div style={{ position: 'relative' }} ref={fieldEditWrapperRef}>
                      <input
                        ...
                        style={{ fontSize: 12.5, fontFamily: FONT, background: 'var(--surface-2)', border: '1px solid var(--border-subtle)', borderRadius: 6, color: 'var(--text-1)', padding: '2px 6px', width: '100%' }}
                      />
                      {renderSuggestionDropdown(idea, 'platform', platformOptions, 26)}
                    </div>
                  ) : (
                    <div onClick={() => startEdit(idea, 'platform')} style={{ fontSize: 12, fontFamily: FONT, color: 'var(--text-3)', cursor: 'text' }} className="truncate">
                  ...
                  {editingField?.id === idea.id && editingField.field === 'category' ? (
                    ...
                    <span onClick={() => startEdit(idea, 'category')} className="truncate" style={{ fontSize: 12, fontFamily: FONT, padding: '3px 10px', borderRadius: 6, background: 'var(--surface-2)', color: 'var(--text-3)', cursor: 'text', width: 'fit-content' }}>
                      {idea.category}
                    </span>
                  ) : (
                    <div onClick={() => startEdit(idea, 'category')} style={{ fontSize: 12, fontFamily: FONT, color: 'var(--text-3)', cursor: 'text' }}>+ Category</div>
                  )}

                  <button
                    onClick={() => handleDocAction(idea)}
                    className="opacity-0 group-hover:opacity-100 transition-opacity"
                    style={{ fontSize: 12, fontFamily: FONT, color: 'var(--accent)', background: 'transparent', border: 'none', padding: '5px 8px', whiteSpace: 'nowrap', cursor: 'pointer' }}
                  >
                  ...
                  <div style={{ position: 'relative' }} ref={menuOpenId === idea.id ? menuRef : undefined}>
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 transition-opacity"
                      style={{ width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 6, color: 'var(--text-3)', background: 'transparent', border: 'none', cursor: 'pointer' }}
                    >
                      <MoreVertical size={15} />
                    </button>
                    {menuOpenId === idea.id && (
                      <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', right: 0, top: 30, zIndex: 50, width: 130, borderRadius: 10, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', padding: '4px 0', overflow: 'hidden', background: 'var(--menu-bg)', border: '1px solid var(--border-subtle)' }}>
                        <button
                          onClick={() => handleDelete(idea)}
                          style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 12px', fontSize: 13, fontFamily: FONT, color: 'var(--danger)', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' }}
                          onMouseEnter={e => (e.currentTarget.style.background = 'var(--hover)')}
                          onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                        >
                ...
                return (
                  <div key={status}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-2)' }}>{STATUS_META[status].label}</span>
                      <span className="text-[11px] font-medium px-1.5 py-0.5 rounded-md" style={{ backgroundColor: 'var(--surface-2)', color: 'var(--text-2)' }}>{columnIdeas.length}</span>
                    </div>
                    {columnIdeas.length === 0 ? (
                      <p className="text-[13px]" style={{ color: 'var(--text-3)' }}>No ideas</p>
                    ) : (
                      <div className="flex flex-col" style={{ gap: 10 }}>
                        {columnIdeas.map(idea => (
                          <div key={idea.id} className="group" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border-subtle)', borderRadius: 10, padding: 12 }}>
                            {editingField?.id === idea.id && editingField.field === 'title' ? (
                              <input
                                ref={editInputRef}
                                value={editValue}
                                ...
                                onBlur={() => commitEdit(idea)}
                                onKeyDown={e => { if (e.key === 'Enter') commitEdit(idea); if (e.key === 'Escape') setEditingField(null) }}
                                style={{ width: '100%', fontSize: 12.5, fontWeight: 500, fontFamily: FONT, background: 'var(--surface-2)', border: '1px solid var(--border-subtle)', borderRadius: 6, color: 'var(--text-1)', padding: '2px 6px', marginBottom: 8 }}
                              />
                            ) : (
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                {idea.type && <span style={{ fontSize: 12, fontFamily: FONT, padding: '2px 8px', borderRadius: 5, background: 'var(--surface-2)', color: TYPE_META[idea.type]?.color ?? 'var(--text-3)' }}>{TYPE_META[idea.type]?.label ?? idea.type}</span>}
                                {idea.platform && <span style={{ fontSize: 12, fontFamily: FONT, padding: '2px 8px', borderRadius: 5, background: 'var(--surface-2)', color: 'var(--text-3)' }}>{idea.platform}</span>}
                                {idea.category && <span style={{ fontSize: 12, fontFamily: FONT, padding: '2px 8px', borderRadius: 5, background: 'var(--surface-2)', color: 'var(--text-3)' }}>{idea.category}</span>}
                              </div>
                              <button
                                onClick={() => handleDocAction(idea)}
                                className="opacity-0 group-hover:opacity-100 transition-opacity"
                                style={{ fontSize: 10.5, fontFamily: FONT, color: 'var(--accent)', background: 'transparent', border: 'none', padding: 0, whiteSpace: 'nowrap', cursor: 'pointer', flexShrink: 0 }}
                              >
